...
export const PassportUpload = (props: CustomProps) => {
    ...
    <Box className={`w-full h-auto overflow-hidden relative`}>
      ...
      >
        <Box
          ...
          className="w-full h-[15em] cursor-cell rounded-xl select-none p-2 max-[850px]:p-1 max-[850px]:h-[12rem] max-[1500px]:h-[15rem] max-[700px]:h-[20rem] max-[500px]:h-[12rem]"
        >
          <Box
            sx={{
              background: file ? `url(${url})` : colors.slate,
            }}
            className="w-full h-full flex items-center justify-center object-cover bg-cover bg-no-repeat rounded-xl"
          >
            <Box
              className={`w-full h-full flex flex-col items-center justify-center hover:opacity-100 rounded-xl ${file ? "opacity-0 hover:bg-[#080909be]" : ""}`}
            >
